    ...
    <div v-show="display == 'block' ? true : false" class="back">
      <div class="bodyAswer">
        ...
        <div class="base_one">
          <h4>1. 計算身體質量指數</h4>
          <h6>
            BMI計算方式=>你的體重:{{ weightV }} / 你的身高:{{ heigh / 100 }}**2
          </h6>
          <h5>
            你的BMI值: <b>{{ bmiValue }}</b>
          </h5>
          <h5>
            肥胖指數: <b>{{ fatValue(bmiValue) }}</b>
          </h5>
        </div>
        <div class="base_two">
          <h4>2. 體脂肪表準表</h4>
          <h6>
            你是: <b>{{ selectedGender }}</b>
          </h6>
          <h6>
            你的年齡:<b>{{ ageFat }}</b
            >歲
...
export default {
  ...
  computed: {
    ...
    bmiValue () {
      ...
      const BMI =
        Math.floor((this.weightV / Math.pow(heightMeter, 2)) * 100) / 100
  ...
  methods: {
    fatValue: function (_bmiValue) {
      if (typeof _bmiValue === 'string') {
        return ''
      } else if (_bmiValue < 18.5) {
        return '體重過輕'
      } else if (_bmiValue >= 18.5 && _bmiValue < 24) {
        return '標準體重'
      } else if (_bmiValue >= 24) {
        if (_bmiValue >= 24 && _bmiValue < 27) {
          return '過重'
        } else if (_bmiValue >= 27 && _bmiValue < 30) {
          return '輕度肥胖'
    ...
    async baseAnswer () {
      ...
      if (!success) {
        ...
        await axios
          ...
          .then(response => {
            axios
              .post('http://localhost:3000/baseTest', {
