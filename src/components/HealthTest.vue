    ...
    healthBtn () {
      ...
      axios
        ...
        .then(response => {
          // 2) chk基本測量是否有做
          axios
            .get('http://localhost:3000/baseTest', {
              params: { caseId: response.data[0].caseId }
            })
            .then(baseTest => {
              axios
                .get('http://localhost:3000/sitTest', {
                  params: { caseId: response.data[0].caseId }
                })
                .then(sitTest => {
                  axios
                    .get('http://localhost:3000/meatTest', {
                      params: { caseId: response.data[0].caseId }
                    })
                    .then(meatTest => {
                      console.log(
                      ...
                      if (baseTest.data[0] === undefined) {
                        if (sitTest.data[0] === undefined) {
                          if (meatTest.data[0] === undefined) {
                            ...
                            alert(
                              '尚未填寫完基本測量、久坐族測驗，無法到下一頁喔~'
                            )
                          }
                        } else {
                          if (meatTest.data[0] === undefined) {
                            alert(
                              '尚未填寫完基本測量、肉食族測驗，無法到下一頁喔~'
                            )
                          } else {
                            alert('尚未填寫完坐久族測驗，無法到下一頁喔~')
                          }
                        }
                      } else {
                        if (sitTest.data[0] === undefined) {
